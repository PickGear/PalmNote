...
data class TravelUiState(val template: LifeTemplate? = null, val items: List<LifeItem> = emptyList(), val isLoading: Boolean = true, val error: String? = null)
...
) : BaseLifeViewModel<TravelUiState>(itemRepo, templateRepo) {
    private var templateJob: Job? = null
    override fun initialState() = TravelUiState()
