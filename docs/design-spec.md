...
| 权限 | 触发时机 | UI 流程 |
|------|---------|---------|
| `ACCESS_FINE_LOCATION` | **规划中**：生活日记获取位置 | 首次使用日记时 → 弹窗说明 → 系统权限弹窗 → 授权后自动获取 |

> **说明：** 位置权限仅用于日记自动获取位置名称和经纬度。天气和温度由用户手动选择/输入，不依赖网络。当前版本（v1.3.0）**未声明** `ACCESS_FINE_LOCATION` 权限，无定位功能；`location` 字段目前为手动填写的文本，自动定位为后续版本规划。
