...
一款**本地优先（local-first）**的生活记录应用，集成记账、资产管理、生活规划与记录、密码本于一体。无需注册，数据全部存储于本地，无需联网即可使用全部功能。
...
- 收支分类、预算设置、月度/年度报表
- 日历视图、高级筛选
- CSV/XLSX 导入（支持支付宝加密 ZIP 直读）、OCR 识别
- 导入账单改版为「分拣台」，待复核项一目了然，支持一键撤销
- 桌面小组件
...
### 🎨 主题与外观
- 多套主题色包 + 自定义颜色
- 6 款渐变壁纸，支持自定义图片壁纸（模糊/透明度可调）
- 应用图标青白配色可选
- 桌面小组件统一圆角卡片，适配深色模式，强调色跟随主题
...
PalmNote 遵循最小权限原则，申请的全部权限如下：

| 权限 | 用途 |
| --- | --- |
| 网络访问 | **未申请**——清单中以 `tools:node="remove"` 显式移除，系统层面不存在该权限 |
| 通知（Android 13+） | 仅用于生活规划的提醒；首次进入主界面时询问一次，拒绝不影响其他功能 |
| 日历读写 | 仅在打开「日历同步」开关时申请，用于将待办/纪念日写入系统日历 |
| 精确闹钟 | 仅用于提醒准点触发；需要时引导到系统设置页手动开启 |
| 开机自启 | 仅用于重启手机后恢复已设定的提醒，不采集任何数据 |

账单、备份与崩溃日志的导入导出全部通过系统文件选择器（SAF）完成，**无需存储权限**；拍照识别账单使用系统照片选择器，**无需相机权限**。

## 已知限制

PalmNote 是纯本地应用，以下限制是"零联网"设计的直接代价，使用前请知悉：

- **本机是唯一副本**：卸载应用或清除数据后记录无法找回，没有云端可以恢复。
- **数据安全靠习惯**：应用内置自动备份（默认关闭，可在「设置 → 数据与备份 → 备份与恢复」开启）与手动备份；备份位置可选网盘同步文件夹。请定期确认备份状态。
...
PalmNote 与微信、支付宝等第三方服务的权利人无任何关联、合作或背书关系。对第三方账单文件（CSV/XLSX/加密 ZIP）的导入仅为在本地对用户自有数据作格式兼容处理，"微信""支付宝"等名称仅作描述性使用，相关商标归其权利人所有。账单解析全程在设备本地完成，不联网、不上传任何数据。
