...
- 首页标题栏同步个人资料（头像/昵称）；时段问候语/提示/名言轮换，长按可切换常驻模式
- 净资产卡片支持自定义颜色，修改即时生效
- 开发规范手册 `docs/DEVELOPMENT.md`（26 章：分支/提交/版本/发布/迁移/安全/日志/无障碍/工作成果保护）
- 备份页新增「记住密码」开关（**默认关闭**）：开启后密码经 Android Keystore 中的密钥包裹保存在本机，下次进入自动预填；明文不落盘，换机或卸载重装后自动失效并回到需重新输入的状态

### Changed
- 界面配色统一：12 处图标灰改走主题 onSurfaceVariant（深浅主题下与整体灰阶一致）；优先级红/橙点、成就解锁绿、报表蓝、倒计时过期琥珀色、评分星黄、布尔勾选绿等 8 处散落硬编码色收敛到主题令牌（新增 LifeAmber/RatingStar）
- 账本选择统一：文件导入预览与 OCR 识别均可选择导入到哪个账本；切换导入方式不再丢失账本列表
- **导出备份必须设置密码，且至少 6 位**：备份文件会保存到您选择的文件夹，可能经网盘/U 盘等渠道流转，未加密或密码过短都会使其中的照片与设置暴露。密码不足 6 位时导出按钮不可用（本机自动备份仍免密，但只能在本机恢复）
- 备份与数据导入/导出的选项描述更准确：备份保存位置、跨机恢复与密码要求说明；"导入数据"明确为合并导入而非覆盖恢复；CSV 导出说明仅含核心数据；恢复对话框仅在加密备份时显示密码输入框
- **数据备份页精简**：两段各 3 行的操作说明与常驻红色警告合并为一行约束提示（"加密保存 · 至少 6 位 · 忘记密码无法恢复"，密码不足时该行转红）；本机自动备份与所选文件夹内的备份合并为一个列表并默认折叠，不再平铺占屏；列表项改为「今天 12:38 · 350 KB」式友好格式，不再显示无信息量的长串文件名
- 组件金额紧凑显示：超过 6 位整数自动折叠（¥12.3万 / ¥123.4k）
- 待办/打卡组件条目整行可点（原 18dp 圆圈热区过小）
- 卡片/主题/壁纸弹窗的取色器统一为同一组件（视觉与交互一致）
- **解锁速度优化**：应用锁 PIN 与密码本的"老参数透明升级"改为后台执行，不再把额外一次 600k 派生叠加在本次解锁等待里（老用户解锁等待约减半）；PIN 校验期间键盘置灰并显示进度指示，消除"点了没反应"的误判；PBKDF2 复用 SecureRandom 实例，省去每次派生前的种子初始化开销
- **区分「备份」与「导出」两条通道**：「数据管理」改名为「数据与存储」，「数据备份」改名为「备份与恢复」；导出与导入的说明改为明确覆盖范围（导出即可读的 CSV-ZIP，只含核心数据；导入为合并、不覆盖现有数据），并指向换机应使用「备份与恢复」
- 导出前新增覆盖范围确认：列出将导出的模块（记账/物品/生活/图片）与**不含**的内容（待办与规划、心情日记、专注记录、成长成就、回收站、密码本），让"我少了什么"在导出前就可知
- 备份列表的折叠入口改为按两类分别计数（本机 / 可换机），不再只给一个总数——本机自动备份只在本机有效，用户需要一眼看出哪些能用于换机
- 恢复确认文案改为准确表述：恢复前会自动保存一份本机快照、可在本机再恢复回来（此前写"此操作不可撤销"，与实际行为不符，会让用户不敢使用恢复）

### Fixed
- **支付宝官方账单文件（CSV/XLSX）此前完全无法导入**：当代导出表头为"交易创建时间"（非旧版"记录时间"），表头匹配不到导致解析 0 条；现兼容新旧两种官方格式，实测 727 条全量识别
- 账单文件导入的分类推断改用商品名回退（备注常为空导致大量误判"其他"）；分类关键词扩充（连锁餐饮/视频娱乐/运营商/电商购物/红包等），实测微信账单"其他"占比 63%→23%（剩余多为"用户xxxx"式无信息转账行）
- 文件导入预览支持逐笔编辑（金额/类型/商户/分类/日期/备注），此前仅能勾选
...
- 备份完整性校验 MD5 → SHA-256
- 备份列表体积恒显示 `0 MB`（350 KB 被整数取整，用户会误以为备份是空的）：改为自适应显示 KB/MB/GB
- 升级前选择已下线图标别名的用户偏好归一化（white_green → cyan_white）
- CSV 导入新增通用格式兜底：银行/云闪付等任意含"时间/日期 + 金额"表头的账单均可导入（#1）
- Excel 导入不再按交易状态白名单过滤丢行，全部行进入预览由用户勾选（#1）
- XLSX 导入识别账单品牌（微信/支付宝），支付宝账单支付方式不再误标为"其他"；CSV/XLSX 均读取交易单号作为精确去重键，同商户同金额同时刻的账单不再被误判重复
...
- 密码本条目图片未纳入备份，恢复后图片丢失（新增 `vault-images/` 打包与解压映射，回滚保护同步覆盖该目录）

### Security
- **导出备份强制加密**：此前导出时可不填密码，产出的明文包含物品/单据/生活照片与 DataStore（内含应用锁 PIN 哈希、最近搜索）等明文条目，仅靠文字提示不足以阻止外流；现在未设置密码即无法导出
- **本机自动备份补上恢复入口**：自动备份落在应用内部存储——其他应用与系统文件管理器都进不去，而恢复界面此前只列出用户选定的文件夹，等于这些备份不可达。现在恢复界面新增「本机备份」分组，可直接列出并恢复
- **备份不再把密钥与密文装进同一个包**：便携数据库密钥 `db_key.txt` 仅在设置了备份密码（加密包）时写入。此前不设密码的明文备份会同时包含 SQLCipher 密文与 Base64 明文主库密钥，加密形同虚设——文件一旦经微信/云盘/U 盘流转即等同全量泄露
- **备份目录由应用专属外部存储改为应用内部存储**：旧目录（`Android/data/<包名>/files/Download/PalmNote`）在 Android 10 及以下可被文件管理器与其他应用读取，且每日自动备份会持续在其中产生新副本；升级后旧备份文件自动迁移至内部存储（只搬不删）
