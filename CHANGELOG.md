...
- 微信/支付宝账单导入（文件或截图）时，钱包自动默认为对应渠道（微信/支付宝），可手动改为其他账户
...
- OCR 识别电商订单页（京东/拼多多/淘宝等）时以"实付款"合计为一笔，不再把"到手价"或被优惠拆开的金额误识别成多笔
...
- **导出备份可能静默失败**：所选文件夹不可写、建文件被拒或输出流打不开时，界面依然提示备份成功，而文件夹里实际没有文件、或只有一个 0 字节空包，用户往往等到换机时才发现导出从未生效。现在这些情况都会明确报错，并清掉中途写坏的不完整文件
- **恢复失败会先关库并留下脏状态**：所选备份文件读不出来时（授权失效/文件已被删），仍会先建恢复前快照并关闭数据库，白跑一趟还多出一份快照；现在读取失败直接中止，不再触碰数据库，且每次恢复前清理缓存残留，避免误用上一次留下的临时包
- 选择备份目录时，若文件管理器不支持持久化授权会直接崩溃；现改为本次导出照常进行，仅目录记忆失效
- 恢复备份的文件选择器只接受 zip/octet-stream，部分手机上 `.palmnote` 会被判为不可选而灰掉；现放宽为任意文件，选错由文件头校验拦下
- 「记住密码」在本机已解不开（换机、卸载重装）后记录仍长期残留，导致开关状态每次都被误判为已记住；现改为读取失败即自清
- 备份页底部提示与恢复确认框文案自相矛盾（一处说"覆盖当前数据"，一处说"可恢复回来"），统一为准确表述
